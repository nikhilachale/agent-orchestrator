...
Built-in reviewer harnesses are `claude-code`, `codex`, `copilot`, `cursor`, `kilocode`, `kiro`, `opencode`, `pi`, and `qwen`. Kilo Code requires the
...
Qwen reviewer support is experimental and host-trusted. AO launches its persistent TUI from a neutral AO-owned directory in
plan mode, but that is not OS isolation: a terminal user can still invoke Qwen's `!` shell or change approval mode.
