...
Built-in reviewer harnesses are `claude-code`, `codex`, `copilot`, `cursor`, `devin`, `droid`, `kilocode`, `kiro`, `opencode`, `pi`, and `qwen`. Kilo Code requires the
...
Qwen, Devin, and Droid reviewer support is experimental and host-trusted. AO always launches their persistent interactive TUIs,
