...
Built-in reviewer harnesses are `claude-code`, `codex`, `copilot`, `cursor`, `devin`, `droid`, `kilocode`, `kiro`, `kimi`, `opencode`, `pi`, and `qwen`. Kilo Code requires the
...
Qwen, Devin, Droid, and Kimi reviewer support is experimental and host-trusted. AO always launches their persistent interactive TUIs,
never their print, exec, headless, or sandbox modes. Their planning and permission defaults reduce accidental model actions, but
terminal users can still invoke shell/editor features or change modes.
Continue, Goose, and Vibe have staged adapters but are not selectable reviewers; they fail closed until their containment and
review-gateway prerequisites are available.
