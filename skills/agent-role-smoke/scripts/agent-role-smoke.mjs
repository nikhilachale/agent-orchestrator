...
async function runOrchestratorSmoke(opts, status, fixture, projectID, harness, createdSessions) {
	...
	try {
		setProjectConfig(opts, projectID, {
			orchestratorRules:
				"AO role smoke orchestrator rule: delegate implementation to a worker; do not edit files in the orchestrator workspace.",
			agentConfig: permissionAgentConfig(opts),
			worker: { agent: harness, agentConfig: permissionAgentConfig(opts) },
...
function cleanupStaleSmokeState(opts, targetHarnesses) {
	...
	for (const session of staleSessions.reverse()) {
		if (session.role === "orchestrator" && session.harness && !target.has(session.harness)) {
			logVerbose(
				opts,
				`stale smoke orchestrator ${session.id} uses ${session.harness}; replacing for ${targetHarnesses.join(",")}`,
			);
		} else {
...
function createFixture() {
	...
	writeFileSync(
		...
		JSON.stringify({ scripts: { test: "node -e \"console.log('fixture ok')\"" } }, null, 2) + "\n",
	);
...
`,
	...
	gitOutput([
		...
		"user.name=AO Smoke",
		"-c",
		"user.email=ao-smoke@example.invalid",
		"commit",
		"-m",
		"initial fixture",
	]);
...
function stopElectron(opts) {
	...
	const patterns = ["Agent Orchestrator", "Electron.*agent-orchestrator", "electron-forge.*agent-orchestrator"];
...
async function waitForFixtureChange(workspace, timeoutMs, opts, sessionID = "") {
	...
	while (Date.now() < deadline) {
		...
		if (existsSync(file)) {
			...
			const changed =
				body.includes("green") ||
				body.includes("#008000") ||
				body.includes("rgb(0, 128, 0)") ||
				body.includes("red") ||
				body.includes("#ff0000");
...
function sendKeysToTmux(opts, sessionID, keys, reason) {
	...
	logVerbose(
		opts,
		`could not send ${keys.join(" ")} to tmux pane ${target}: ${res.stderr || res.stdout || `exit ${res.status}`}`,
	);
}
...
async function sendTaskToOrchestrator(opts, sessionID, message, marker) {
	...
	throw new Error(
		`task message marker ${marker} did not appear in orchestrator ${sessionID} pane after send.${extra}\nRecent pane output:\n${pane}`,
	);
}
...
function pretrustCursorWorkspace(opts, workspacePath) {
	...
	writeFileSync(trustPath, JSON.stringify({ trustedAt: new Date().toISOString(), workspacePath }, null, 2) + "\n", {
		mode: 0o644,
	});
